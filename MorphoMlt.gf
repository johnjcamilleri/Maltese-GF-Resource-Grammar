  ...
  oper
    ...
    verbPolarityTable : VerbInfo -> (VForm => VSuffixForm => Str) -> (VForm => VSuffixForm => Polarity => Str) = \info,tbl ->
      ...
      let
        ...
        -- TODO: rename?
        patt2 = vowelChangesIE info.root info.vseq ;

        -- First some pre-processing of stem
        s2 : Str = case <info, s> of {
            <{form=FormIII}, (w@#Cns)+"ie"+(g@#Cns)+"e"+(b)> => w+(patt2.V1)+g+(patt2.V2)+b ; -- WIEĠEB > WIĠIB-
            <{form=FormIII}, (n@#Cns)+(w@#Cns)+"ie"+(g@#Cns)+"e"+(b)> => n+w+(patt2.V1)+g+(patt2.V2)+b ; -- NWIEĠEB > NWIĠIB-
            <{form=FormIII}, (q@#Cns)+"ie"+(ghdek)> => q+(patt2.V1)+ghdek ; -- QIEGĦDEK > QEGĦDEK-
            <{form=FormIII}, (n@#Cns)+(q@#Cns)+"ie"+(ghdek)> => n+q+(patt2.V1)+ghdek ; -- NQIEGĦDEK > NQEGĦDEK-
            _ => s
          } ;
      in table {
        Pos => s ;
    ...
    verbPerfPronSuffixTable : VerbInfo -> (VAgr => Str) -> (VAgr => VSuffixForm => Str) = \info,tbl ->
      ...
      table {
        ...
        AgP3Pl => -- Huma FETĦU
          let
            fethu = ie2_ (patt2.V1) (tbl ! AgP3Pl) ;
          in
          table {
...
-- ========================================================================
    ...
    verbImpStem : VerbInfo -> Str -> Str = \info,iftah ->
      let
        vowels : Vowels = extractVowels info.imp ;
--        iftah : Str = info.imp ; --- is there no way to avoid the iftah parameter?
      ...
      case <info.form, info.class> of {
        ...
        <FormVII, _> => info.root.C1 + info.vseq.V1 + info.root.C2 + info.root.C3 ; -- -NĦASL
        <FormVIII, _> => info.root.C1 + info.vseq.V1 + info.root.C2 + info.root.C3 ; -- -NTEFAQ
        <FormIX, _> => info.root.C1 + info.root.C2 + info.vseq.V1 + info.root.C3 ; -- -ĦDAR
...
-- ------------------------------------------------------------------------
    ...
    -- Imperfective tense
    verbImpfPronSuffixTable : VerbInfo -> (VAgr => Str) -> (VAgr => VSuffixForm => Str) = \info,tbl ->
      let
        vowels : Vowels = extractVowels info.imp ;
        ...
        iftah : Str = case dropPfx 1 (tbl ! AgP1 Sg) of {
          ...
          str+"ie"+h => str+(patt2.V1)+h ; -- -STRIEĦ > -STRIĦ --- to general?
...
--            str + "ie" + b@#Cns => "i" + info.imp ; -- -ISTRIĦ
      ...
      table {
            ...
            VSuffixInd agr =>
              let
                tifthi = case <info, vowels.V1> of {
                  <{form=FormIII}, "ie"> => (ie2_ patt2.V1 tifth) + "i" ; -- TWIĠIB-
            ...
            VSuffixInd agr =>
              let
                jifthi = case <info, vowels.V1> of {
                  <{form=FormIII}, "ie"> => (ie2_ patt2.V1 jifth) + "i" ; -- JWIĠIB-
        ...
        AgP1 Pl => -- Aħna NIFTĦU
          let
            nifthu = ie2_ patt2.V1 (tbl ! AgP1 Pl) ;
          in
          table {
        ...
        AgP2 Pl => -- Intom TIFTĦU
          let
            tifthu = ie2_ patt2.V1 (tbl ! AgP2 Pl) ;
          in
          table {
        ...
        AgP3Pl => -- Huma JIFTĦU
          let
            jifthu = ie2_ patt2.V1 (tbl ! AgP3Pl) ;
          in
          table {
            VSuffixNone => tbl ! AgP3Pl ;
            VSuffixDir agr =>
...
-- ========================================================================
    ...
    verbDirIndSuffixTable : VAgr -> GenNum -> Str -> (VAgr => Str) = \subj,do,ftaht ->
      case do of {
        GSg Masc => table {
    ...
    conjFormII : VerbInfo -> (VForm => VSuffixForm => Polarity => Str) = \i ->
      let
        ...
        info : VerbInfo = mkVerbInfo i.class FormII i.root i.vseq (imp ! Sg) ;
